...
const sidebarLinks = [
  ...
  { icon: Briefcase, label: 'Projects', href: '/freelancer/projects' },
  { icon: Send, label: 'Proposals', href: '/freelancer/proposals' },
...
export default function FreelancerSidebar() {
  ...
  const displayName = (user as any)?.fullName || user?.name || 'Freelancer';
    ...
    <aside className="hidden lg:flex w-64 flex-col bg-card border-r border-border/40 h-full flex-shrink-0">
      ...
      <div className="p-6 border-b border-border/40">
        <Link href="/" className="flex items-center gap-3 group">
          ...
          <div>
            <p className="font-extrabold text-foreground text-sm tracking-tight">Freelancer Hub</p>
      ...
      <nav className="flex-1 p-4 space-y-1.5 overflow-y-auto">
        {sidebarLinks.map((link) => {
          ...
          const isActive = pathname === link.href || (link.href !== '/freelancer/dashboard' && pathname.startsWith(link.href));
      ...
      <div className="p-4 border-t border-border/40 space-y-2">
        <div className="flex items-center gap-3.5 p-3 rounded-xl bg-muted/45 border border-border/20 shadow-inner">
          ...
          <div className="flex-1 min-w-0">
            ...
            <p className="text-[10px] font-semibold text-muted-foreground mt-0.5">Verified Partner</p>
          </div>
        </div>
